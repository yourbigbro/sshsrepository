...
public class CustomerAction extends ActionSupport implements ModelDriven<Customer>{
	...
	//将后台封装好的pageBean变量传到前台jsp页面进行数据显示
	...
	//将封装好的pageSize和currentPage传入方法当参数，进入service层之后再调用PageBean，因为分页和事务是存在于service层之中的
	//这个方法取代的应该是返回到list页面展示数据的list()方法
	...
	public String list(){
		...
		//此处没给criteria对象传入任何参数，所以取默认值，也就是查询所有记录，即select * from cust_customer
		...
		//为全局私有变量pageBean赋值，使得页面取到该值
		pageBean=customerService.listCurrentPageCustomer(criteria,currentPage,pageSize);
